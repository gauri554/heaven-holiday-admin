...
import { itineraryListData,bannerListData,flightData, accomodationData,tourinfoData,reviewsData,citiesListData, departureListData,ordersData, bookingData, permissionsList, packageData,reviewListData, roleListData, socialGroupsData, userData, warehouseData } from '@/assets/data/products';
...
export const getTourInfoData = async () => {
  return tourinfoData;
};
...
export const getDepartureData = async () => {
  return departureListData;
};
...
export const getPackageById = async id => {
  const data = packageData.find(pkg => pkg.id == id);
  await sleep();
  return data;
};
...
export const getAllOrders = async () => {
  const data = ordersData.map(order => {
    ...
    const pkg = packageData.find(pkg => pkg.id === order.packageId);
    return {
      ...order,
      customer,
      package: pkg
...
export const getAllBooking = async () => {
  const data = bookingData.map(booking => {
    const customer = userData.find(user => user.id === booking.customerId);
    const pkg = packageData.find(pkg => pkg.id === booking.packageId);
    return {
      ...booking,
      customer,
      package: pkg
